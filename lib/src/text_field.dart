...
class _CustomTextFieldState extends State<CustomTextField> with RestorationMixin {
  late TextEditingController _textController = widget.controller ?? TextEditingController();
  ...
  CustomKeyboardConnection? _currentConnection;

  CustomKeyboardConnection get _connection => _currentConnection ??= CustomKeyboardConnection(
        name: inputType.name,
  ...
  KeyboardWrapperState get _keyboardWrapper {
    final keyboardWrapper = KeyboardWrapper.of(context);
    if (keyboardWrapper == null) throw KeyboardWrapperNotFound();
    return keyboardWrapper;
  }

  CustomTextInputType get inputType {
    if (widget.keyboardType is CustomTextInputType) return widget.keyboardType as CustomTextInputType;
    return const CustomTextInputType(name: 'ityhoang');
  }

  bool get _isCustomKeyboard => widget.keyboardType is CustomTextInputType;
  ...
  void didUpdateWidget(CustomTextField oldWidget) {
    ...
    if (oldWidget.controller != widget.controller) {
      _textController = widget.controller ?? _textController;
    }
  ...
  void _triggerCursor(String newText) {
  ...
  void _triggerOnChanged() {
    _triggerCursor(_textController.text);
    widget.onChanged?.call(_textController.text);
  }

  void _onTap() {
    if (_isCustomKeyboard) {
      if (!_keyboardWrapper.manager.isKeyboardOpen && FocusScope.of(context).hasFocus) FocusScope.of(context).unfocus();
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      keyboardType: _isCustomKeyboard ? TextInputType.none : widget.keyboardType,
