...
class KeyboardWrapperState extends State<KeyboardWrapper> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animationController.addListener(() {
      double keyboardPosition = _animationPosition.value.dy;
      _resizeHeightKeyBoard.value = (_keyboardHeight + (widget.removePaddingSafe ? 0 : MediaQuery.of(context).padding.bottom)) * (1 - keyboardPosition);
    });
    super.initState();
  }
    ...
    return RepaintBoundary(
      child: MediaQuery(
        data: _activeKeyboard != null
            ? data.copyWith(
                viewInsets: widget.removeViewInsetsBottom ? null : data.viewInsets.copyWith(bottom: _bottomInset + data.viewInsets.bottom + data.padding.bottom),
              )
            : data,
        child: Stack(
          children: [
            widget.isBottomSheetOrDialog ? _buildPaddingForDialogOrSheet() : _buildPositionedForNormal(),
  ...
  Widget _buildPaddingForDialogOrSheet() {
    return ValueListenableBuilder<double>(
      ...
      builder: (context, height, child) {
        return Padding(
          padding: EdgeInsets.only(bottom: widget.hasPaddingKeyboard ? (max(0, height - max(0, widget.heightBottomNavigationBar))) : 0),
          child: widget.child,
        );
      },
    );
  ...
  void onKey(CustomKeyboardEvent key) {
    ...
    void replaceSelectionFormat({TextSelection? selection, String newText = ""}) {
    ...
    void replaceOperator({TextSelection? selection, String newText = ""}) {
      var originalValue = keyboardConnection.controller.value;
      final selectionToUse = selection ?? originalValue.selection;
